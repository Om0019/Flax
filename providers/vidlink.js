  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (parsedStreams.length === 0) {
        ...
        console.log(`[Vidlink] Found ${parsedStreams.length} quality variants, returning adaptive stream`);
      }
      return [{
        name: "Vidlink",
  ...
  try {
    ...
    if (data.stream && data.stream.qualities) {
      if (data.stream.playlist) {
        console.log(`[Vidlink] Processing adaptive playlist from stream object`);
        streams.push({
          _isPlaylist: true,
        ...
        Object.entries(data.stream.qualities).forEach(([qualityKey, qualityData]) => {
          if (qualityData.url) {
            ...
            streams.push({
              ...
              title: streamTitle,
              url: qualityData.url,
              type: qualityData.url.includes(".m3u8") ? "hls" : void 0,
              quality,
              size: "Unknown",
              headers: VIDLINK_HEADERS,
              provider: "vidlink"
      ...
      streams.push({
        ...
        title: streamTitle,
        url: data.url,
        type: data.url.includes(".m3u8") ? "hls" : void 0,
        quality,
        size: "Unknown",
        headers: VIDLINK_HEADERS,
        provider: "vidlink"
      ...
      data.streams.forEach((stream, index) => {
        if (stream.url) {
          ...
          streams.push({
            ...
            title: streamTitle,
            url: stream.url,
            type: stream.url.includes(".m3u8") ? "hls" : void 0,
            quality,
      ...
      data.links.forEach((link, index) => {
        if (link.url) {
          ...
          streams.push({
            ...
            title: streamTitle,
            url: link.url,
            type: link.url.includes(".m3u8") ? "hls" : void 0,
            quality,
